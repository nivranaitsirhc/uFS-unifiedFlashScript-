...
link_files(){
	...
	local LSO_DIR=;local LIB_DIR=
	list_link | while read A B;do
		[ -e "$A" ] && {
			LSO_DIR=$(dirname $B);
			LIB_DIR=$(echo $B | cut -d/ -f 1-4)
			[ ! -e "$LSO_DIR" ] && {
				install -d "$LSO_DIR"
				for L in $(find "$LIB_DIR" -type d);do
					set_perm 0 0 755 "$L"
				done
			}
			$LT3 "I: ASLIB link_files: Restoring File Link - $B"
			ls -sfn "$A" "$B"
		}
	done
}
...
ch_con_ext() { # sub-function of set_perm
	...
	! LD_LIBRARY_PATH=$SYSTEMLIB /system/bin/toolbox chcon $2     && \
	chcon $2 $1
}
...
is_mounted() {
	...
	[ -n "$2" ] && {
		cat /proc/mounts | grep -m 1 $1 | grep -m 1 $2,	>/dev/null 2>&1
		s=$?
	} || {
		cat /proc/mounts | grep -m 1 $1				>/dev/null 2>&1
		s=$?
	}
	[ "$s" == "0" ] && \
  ...
  post-restore)
	...
	for i in $(list_files); do
		[ -e "$S/$i" ] && {
			...
			DN="$(dirname "$S/$i")"
			chmod 0755 "$DN" && \
			file_log "I: $addon_name: dir set to 755 -> $DN" || \
